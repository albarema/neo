...
rule all:
    input:
        #expand("paneldir/{panel}-{level}.acf.gz", level=LEVEL, panel='euras.oldbatch.missinfopass'),
        expand("UKBiobank/selection_UKBV2/{panel}/Genscores-{level}-50_irnt.txt", panel='euras.oldbatch.missinfopass',level=LEVEL)
...
rule get_filtered_vcf:
    input:
        vcf="vcf/euras-qx-infopass.vcf.gz",
    ...
    output:
        vcft=temp("vcf/{panel}.tmp.qx.vcf.gz"),
        passed="vcf/{panel}.qx.vcf.gz"
    shell:
        """
        bcftools view -Ou {input.vcf} | bcftools +setGT -Oz -- -t q -n . -e 'GP[*] >= 0.9' > {output.vcft}
        /willerslev/users-shared/science-snm-willerslev-gsd818/Applications/vcftools/bin/vcftools --gzvcf {output.vcft} --keep {input.inds} --recode --stdout | bgzip -c > {output.passed}

...
## --------------------------------------------------------------------------------
##### global parameters #####
#PHENOS=pd.read_table('phenotypes_qx_europeGP08pass_sig_BC.txt')['phenotype'].tolist()
PHENOS=["23105_irnt", "23130_irnt", "6144_3", "C44"]
PANNAME=['europe']
## --------------------------------------------------------------------------------
#####
rule all:
    input:
        "UKBiobank/data/allind/weightedFreqs_candidates-inds-50_irnt.tsv",
        # genscores
        #expand("UKBiobank/selection_UKBV2/{panel}/Genscores-inds-{pheno}.txt", pheno=PHENOS, panel=PANNAME),
        # pcaplot
        expand("pca/plots/{panel}/neo.impute-{panel}-k{k}-pca.plot_by{pheno}_5e-8.nolabels.png", pheno=PHENOS, k=2, panel=PANNAME),
        # maps
...
rule get_can:
    input:
        ...
        lbd=config['lbd']
    output:
        can="UKBiobank/data/allind/weightedFreqs_candidates-{level}-{pheno}.tsv",
    shell:

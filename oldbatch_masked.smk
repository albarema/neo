...
rule all:
    input:
        #expand("paneldir/{panel}-{level}.acf.gz", level=LEVEL, panel='euras.oldbatch.miss'),
        expand("UKBiobank/selection_UKBV2/{panel}/Genscores-{level}-50_irnt.txt", level=LEVEL, panel='euras.oldbatch.miss')
...
rule get_filtered_vcf:
    ...
    output:
        vcf=temp("vcf/{panel}.tmp.qx.vcf.gz"),
        passed="vcf/{panel}.qx.vcf.gz"
    shell:
        """
        bcftools view -Ou {input.vcf} | bcftools +setGT -Oz -- -t q -n . -e 'GP[*] >= 0.9' > {output.vcf}
        /willerslev/users-shared/science-snm-willerslev-gsd818/Applications/vcftools/bin/vcftools --gzvcf {output.vcf} --keep {input.inds} --positions {input.rmid} --recode --stdout | bgzip -c > {output.passed}
...
rule get_panel_galact:
    input:
        ...
        panelfile="/willerslev/users-shared/science-snm-willerslev-gsd818/neo/20200309-impute/euras.{level}.panelfile_V2.txt"
    output:
